...
class LatentDataset(Dataset):
    ...
    Note: text_ids is deleted (unlike Flux which uses them)
    """
    def __init__(
        ...
        # Initialize uncond embeddings with correct dimensions for SD3.5
        # Will be set properly when first sample is loaded
        self.uncond_prompt_embed = None
        self.uncond_pooled_prompt_embed = None
    ...
    def __getitem__(self, idx):
        ...
        # Initialize uncond embeddings with correct shapes on first load
        if self.uncond_prompt_embed is None:
            self.uncond_prompt_embed = torch.zeros_like(prompt_embed)
            self.uncond_pooled_prompt_embed = torch.zeros_like(pooled_prompt_embeds)

        # Apply CFG dropout
        if random.random() < self.cfg_rate:
            prompt_embed = self.uncond_prompt_embed
            pooled_prompt_embeds = self.uncond_pooled_prompt_embed
...
def latent_collate_function(batch):
    ...
    Note: text_ids is deleted (unlike Flux which needs it)
    """
